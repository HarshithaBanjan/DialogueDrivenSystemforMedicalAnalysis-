...
# Simple helper to translate text to the user's selected language
def t(text, dest=None):
    """Translate `text` to dest language code. If dest is None use session lang.
       If destination is English or text is short, returns original text to avoid bad translations.
    """
    if not text:
        return text
    dest_code = dest if dest else st.session_state.get('lang', 'en')
    # If destination is English or dest is same as source, return original
    if dest_code == 'en':
        return text
    try:
...
# Language selector in sidebar (global)
st.sidebar.title("🌐 Language")
selected_lang_name = st.sidebar.selectbox("Select your preferred language:", list(LANGUAGE_OPTIONS.keys()), index=0)
st.session_state.lang = LANGUAGE_OPTIONS[selected_lang_name]

# Also show patient info if logged in
...
# Helper: translate and optionally fallback
...
# Medical knowledge base (English - will be translated for display)
MEDICAL_CONDITIONS = {
    "diabetes": {
...
# Patient Login Component with language support
